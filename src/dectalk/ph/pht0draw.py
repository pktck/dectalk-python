...
Translated from ``src/dapi/src/ph/ph_drwt02.c`` lines 765-1507
(HLSyn-enabled variant; ``ph_drwt01.c`` is the FAKE_HLSYN variant).
...
High-level flow (MALE path only; FEMALE is shimmed):
...
5. **Segmental tracking**: Advance ``np_drawt0``/``phocur`` whenever
   ...
   in ``us_f0msegtars`` and set ``tarseg`` / ``tarseg1``.
...
Skipped branches (shimmed with ``NotImplementedError``):

- ``FEMALE`` path (``pDph_t->malfem != MALE``) — ~700 lines,
  C source lines 1508-1978.
- ``SINGING`` / ``PHONE_TARGETS_SPECIFIED`` / ``TIME_VALUE_SPECIFIED``
  are handled by delegation to :func:`~dectalk.ph.linear_interp.linear_interp`
  / :func:`~dectalk.ph.set_user_target.set_user_target` (already ported).
- Non-US-English language table branches (BRITISH, GERMAN, FRENCH,
  SPANISH, LATIN) — fall through to US tables as in the C default.
"""
...
from dectalk.ph.us_f0_segtars import us_f0msegtars
...
def pht0draw(ph_tts: TtsHandle) -> None:  # noqa: PLR0912, PLR0915
    ...
    Only the **MALE** path (``pDph_t->malfem == MALE``) is ported.
    Calling with a FEMALE voice raises ``NotImplementedError``.
    ...
    fall back to ``us_f0msegtars`` (matching the C default).
    ...
    Raises:
        NotImplementedError: When ``pDph_t->malfem != MALE``.
            The FEMALE branch is ~700 lines in the C source
            (ph_drwt02.c lines 1508-1978) and will be ported
            in a follow-up task.
    """
    ...
    # C: pKsd_t = phTTS->pKernelShareData; (not used in HLSYN MALE path)

    # -----------------------------------------------------------------------
    # FEMALE path — not ported yet.
    # -----------------------------------------------------------------------
    if p_dph_t.malfem != MALE:
        raise NotImplementedError(
            "pht0draw: FEMALE path not yet ported. See ph_drwt02.c lines 1508-1978."
        )
