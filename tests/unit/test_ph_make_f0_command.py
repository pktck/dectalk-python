"""Verify make_f0_command matches ph_inton2.c."""
...
def test_first_command_appends_to_arrays() -> None:
    """A first command appends to all four f0 arrays at index 0."""
    state = DphT()
    cumdur = [0]
    make_f0_command(
...
def test_ps_cumdur_reset_to_neg_delay() -> None:
    ...
    cumdur = [42]
    make_f0_command(
        state, f0_type=0, rulenumber=0, tar=0, delay=8, length=0, ps_cumdur=cumdur, nphon=0
    )
    assert cumdur[0] == -8
...
def test_two_commands_appended_in_order() -> None:
    ...
    assert state.f0type[0] == 1
    assert state.f0type[1] == 2
    assert state.nf0tot == 2


def test_buffers_grow_to_nphon_max() -> None:
    """The four f0 arrays grow to ``NPHON_MAX`` entries on first call."""
    state = DphT()
    cumdur = [0]
    make_f0_command(
        state, f0_type=0, rulenumber=0, tar=0, delay=0, length=0, ps_cumdur=cumdur, nphon=0
    ...
    assert len(state.f0type) == NPHON_MAX
    assert len(state.f0length) == NPHON_MAX
