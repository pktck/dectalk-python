...
def test_arpabet_words_to_symbols_leading_wbound_no_sil() -> None:
    """The encoder seeds ``symbols[]`` with a leading WBOUND only — no GEN_SIL.

    Per issue #139 (Klatt frame audit refresh PR #137 finding G), the
    Python port previously prepended ``GEN_SIL`` to mimic the C kernel's
    ``ph_task.c`` line 437 ``pDph_t->symbols[0] = GEN_SIL`` task-init
    bootstrap. That bootstrap is one-time (before the FIRST clause); the
    C binary's per-clause flow does not re-emit a leading silence phone.
    The Python prepend translated through ``make_phone`` →
    ``us_phalloph`` into ``allophons[0] = GEN_SIL`` with the default
    ~15-frame minimum duration, bleeding wrong defaults into the audio
    output. The encoder now emits only ``all_phsort``'s defensive
    leading-WBOUND (C lines 493-495).
    """
    from dectalk.include.phoneme_codes import WBOUND  # noqa: PLC0415

    syms, n = _arpabet_words_to_symbols([["AH0"]], is_sentence_final=True)
    assert n == len(syms)
    assert syms[0] == WBOUND, (
        f"expected leading WBOUND, got {syms[0]:#x} (leading GEN_SIL regression — see issue #139)"
    )
    # No GEN_SIL anywhere in the encoded stream for a single-phone clause
    # (the trailing-silence pad is generated by us_phalloph's sentinel
    # write at ``allophons[nallotot]``, not by an explicit symbol).
    assert GEN_SIL not in syms, (
        f"unexpected GEN_SIL in symbol stream: {syms} (issue #139 regression)"
    )
...
def test_phalloph2_populates_allophons_for_hello_world() -> None:
    """``phalloph2`` writes the full allophone sequence with a trailing sentinel.
    ...
    handles the trailing-silence sentinel itself (``us_phalloph`` line
    509). The previously-prepended leading ``GEN_SIL`` was removed in
    issue #139: the C binary's first emitted frame is the first real
    phone (e.g. ``US_HX`` for "hello"), not silence.
    """
    ...
    # PERIOD-induced trailing-SIL bookkeeping.
    assert p_dph_t.nallotot >= 8, (
        f"nallotot={p_dph_t.nallotot}; expected >= 8 phones for 'hello world'"
    )
    # allophons[0] should be the first REAL phone (no leading-SIL
    # prepend after issue #139). For "hello" the chain emits HX first.
    assert p_dph_t.allophons[0] != GEN_SIL, (
        f"allophons[0]={p_dph_t.allophons[0]:#x} is GEN_SIL — leading "
        "GEN_SIL prepend regression (see issue #139)"
    )
