...
def test_arpabet_words_to_symbols_leading_sil_then_wbound() -> None:
    """The encoder seeds ``symbols[]`` with a leading GEN_SIL then WBOUND.

    Per issue #200 (restoration of leading silence regression caused by
    issue #139), the encoder must prepend the leading ``GEN_SIL``
    phoneme to mirror ``ph_task.c`` lines 437-439's per-clause
    ``symbols[0] = GEN_SIL`` bootstrap. Without it, the first emitted
    Klatt frame is the first content phone (e.g. ``HX`` for ``hello``)
    and the C reference's 213-sample leading-zero prefix collapses to
    a 2-sample bleed. The C binary's per-clause ``say -a`` flow does
    re-emit the leading silence phone — the SIL phone supplies the
    leading silence that fills the audio buffer before the first
    voiced sample.
    """
    from dectalk.include.phoneme_codes import PFUSA, WBOUND, USPhoneme  # noqa: PLC0415

    syms, n = _arpabet_words_to_symbols([["AH0"]], is_sentence_final=True)
    assert n == len(syms)
    expected_sil = (PFUSA << 8) | int(USPhoneme.SIL)
...
def test_phalloph2_populates_allophons_for_hello_world() -> None:
    """``phalloph2`` writes the full allophone sequence with leading + trailing SIL.

    The phalloph2 chain handles the trailing-silence sentinel itself
    (``us_phalloph`` line 509). Per issue #200 (reversal of #139),
    the leading-SIL prepend is restored to mirror the C binary's
    per-clause behaviour — the C reference WAV for ``hello world``
    starts with 213 samples (~3 Klatt frames) of zeros, sourced from
    this leading-SIL phone.
    """
    ...
    # PERIOD-induced trailing-SIL bookkeeping plus the leading SIL.
    assert p_dph_t.nallotot >= 9, (
        f"nallotot={p_dph_t.nallotot}; expected >= 9 phones for 'hello world' "
        "(7 content + leading-SIL + trailing-SIL)"
    )
    # allophons[0] must be the leading GEN_SIL (issue #200): the C
    # reference's 213-sample leading silence is sourced from this
    # phone's duration in the per-frame driver loop.
    assert p_dph_t.allophons[0] == GEN_SIL, (
        f"allophons[0]={p_dph_t.allophons[0]:#x} is not GEN_SIL — "
        "leading GEN_SIL prepend missing (see issue #200)"
    )
