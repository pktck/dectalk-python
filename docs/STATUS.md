...
### State of the workflow overhaul (PR #2)

Shipped and verified by green ci-full on `cdc37f3`:

- `ci-fast.yml` (per-commit, ubuntu + Py 3.11) and `ci.yml` (full
  matrix, `main`/`dev` + PR), both with `paths-ignore` for doc-only
  changes and `concurrency: cancel-in-progress: true`.
- `build-c-oracle.yml` publishes prebuilt oracle tarballs as GitHub
  Releases when the build inputs change. `scripts/setup_c_oracle.sh`
  is SHA-pinned + release-fetch-first + source-build fallback.
- `pr-review.yml` posts a sticky review comment on PRs, bounded to
  the last push's diff so review cost stays linear.
- `scripts/agent_oracle_env.sh` for per-agent oracle isolation
  (`/tmp/dectalk-src-<slug>`) so parallel translators don't collide.
- pytest markers `c_oracle` / `parity` / `perf`, `pytest-xdist` with
  `-n auto`, and `dev_check.sh` modes `--changed` and `--smoke`.
- `docs/TASKS.md` task queue (auto-generated by
  `scripts/refresh_tasks.py`), `docs/PORTING.md` per-task playbook,
  `scripts/scaffold_parity_test.py` for parity-test boilerplate.
- `.pre-commit-config.yaml` (ruff + ruff-format + pyright).
- Branch protection on `main` and `dev` via REST API: four required
  checks (`Lint + type-check + audio diagnostic (Ubuntu, Py 3.11)`,
  `Pytest (ubuntu-latest, Py 3.11)`, `Shellcheck`, `Tests with C
  library (Ubuntu, Py 3.11)`), strict mode, no force-push, no
  deletion, linear history required.
- `CLAUDE.md` updated with CI-watch protocol, throttling rules,
  parallelization patterns, branch protection notes, dev_check modes,
  and Phase E identification as the active blocker.
...
1. Take PR #2 out of draft and merge into `dev` once review passes.
   Branch protection's strict mode means head must be up to date with
   `dev` before merge; rebase rather than merge-commit (linear history
   is required).
2. Start the Phase E PH-stage port. `docs/TASKS.md` has 12 open `ph/`
   targets (`phsettar.py`, `getbegtar.py`, `getendtar.py`, `gettar.py`,
   `init_variables.py`, `make_dip.py`, `phinton.py`, `all_phsort.py`,
   `fr_phsort.py`, `setloc.py`, plus 2 `cmd/` ports). Follow
   `docs/PORTING.md` per-task playbook. Recommended starting point:
   `phsettar.py` (target-area setup) — it's a leaf in the PH call
   graph, so it can land without depending on other PH ports.
3. After every push to a `claude/**` branch, subscribe to the PR via
   `mcp__github__subscribe_pr_activity` and continue working — do not
   block on CI. See `CLAUDE.md` §"CI watch".
